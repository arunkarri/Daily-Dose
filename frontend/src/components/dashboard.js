...
const Dashboard = () => {
  ...
  async function getBlogs() {
    setIsLoading(true);
    const req = await fetch(`${env}blogs`);
                  ...
                  <div className="card-body">
                    ...
                    <div className="text-center">
                      ...
                      {ele.tags.map((tag, tagIndex) => {
                        return <span key={tagIndex} className="badge badge-orange mt-2">{tag}</span>;
                      })}
                    </div>
                    <hr />
                    <p className="card-text text-center">
                      Author: <span className="badge badge-success">{ele.name}</span>
                    </p>
                    <p className="card-text text-center">
